...
class TestUsersBackendMethods(unittest.TestCase):
    ...
    def test_client_add_remove_method(self):
        data = {"login": "ozon", "password": self.password, "type": 0, "firm_id": 1}
        headers = {"Content-type": "application/json"}
        result = requests.post(self.add_method_url, data=json.dumps(data), headers=headers)
        self.assertEqual(result.status_code, 400)
        data["type"] = 1
    ...
    def test_user_add_get_method(self):
        ...
        result = requests.post(self.add_method_url, data=json.dumps(data), headers=headers)
        self.assertEqual(result.status_code, 200)
        result = requests.get(self.get_method_url+"?login=ivan&type=1")
        self.assertEqual(result.status_code, 400)
    ...
    def test_client_add_get_method(self):
        data = {"login": "ozon", "password": self.password, "type": 1, "firm_id": 1}
        headers = {"Content-type": "application/json"}
        result = requests.post(self.add_method_url, data=json.dumps(data), headers=headers)
        self.assertEqual(result.status_code, 200)
        result = requests.get(self.get_method_url+"?login=ozon&type=0")
        self.assertEqual(result.status_code, 400)
    ...
    def test_user_update_method(self):
        ...
        result = requests.put(self.update_method_url, data=json.dumps(data), headers=headers)
        self.assertEqual(result.status_code, 200)
        result = requests.get(self.get_method_url + "?login=ivan&type=0")
        self.assertEqual(result.status_code, 200)
        result = result.json()
    ...
    def test_client_update_method(self):
        ...
        result = requests.put(self.update_method_url, data=json.dumps(data), headers=headers)
        self.assertEqual(result.status_code, 200)
        result = requests.get(self.get_method_url + "?login=ozon&type=1")
        self.assertEqual(result.status_code, 200)
        result = result.json()
